#### 1. 登陆您的控制面板，找到您要推广的应用，选择“Tracker URLs”
![image](imgs/001adjust.png)
#### 2. 点击“SKIP”，进入下一步，开始创建监测链接
![image](imgs/001adjustapptrack.png) 
#### 3. 选择“NEW TRACKER”创建新的监测链接
![image](imgs/001adjustnewtracker.png) 
#### 4.在TRACKER NAME中输入您创建的监测链接的名称，在NETWORK中选择ZPLAY Ads，选择“CUSTOMIZE”
![image](imgs/001adjustnewtracker1.png) 
#### 5. 可以获取CLICK URL与IMPRESSION URL
![image](imgs/001adjustnewtracker2.png) 
#### 6.	在您创建广告或者编辑广告将获取到的URL填写到ZPLAY Ads的监测地址框内 
![image](imgs/000zplay.png)
